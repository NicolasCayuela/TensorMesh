...
def _faces_from_facets(f: torch.Tensor) -> np.ndarray:
    parts = []
...
def _render_3d_compare(plotter: pv.Plotter, row: int, col: int, element_cls, order: int, gmsh: bool):
    ...
    facets = element_cls.get_facet(order=order)
    if isinstance(facets, tuple):
        # Mixed facets (not expected in this script currently), flatten
        facets = torch.cat([f for f in facets], dim=0)
    facets = facets.to(torch.long)
    ...
    if not gmsh:
        ...
        plotter.subplot(row, col)
        plotter.add_text(f"{title}\nTensorMesh/FEniCS", font_size=10)
        # Don't show edges: high-order facets are tessellated for rendering and edges look like "internal cuts".
        plotter.add_mesh(surf, color="lightgray", show_edges=False, opacity=0.95)
        _add_labels(plotter, surf)
    ...
    inv_internal_to_gmsh = torch.argsort(perm)
    basis_gmsh = basis_internal.index_select(0, perm)
    facets_gmsh = inv_internal_to_gmsh[facets]

    pts = _to_xyz(basis_gmsh)
    faces = _faces_from_facets(facets_gmsh)
    surf = pv.PolyData(pts, faces)
    plotter.subplot(row, col)
    plotter.add_text(f"{title}\nGmsh/VTK", font_size=10)
