...
# --------------------------------------------------------------------------- #
def compute_bands(problem: SquareCrystal, verbose: bool = True) -> Dict[str, Any]:
    ...
    for i, k in enumerate(ks):
        ...
        mu = generalized_eigh(Kr.to_dense(), Mr.to_dense())
        mu = torch.clamp(mu[:problem.n_bands].real, min=0.0)
        freqs[i] = (problem.c * torch.sqrt(mu) / (2 * math.pi)).cpu().numpy()
...
# --------------------------------------------------------------------------- #
def plot_bands(result: Dict[str, Any], problem: SquareCrystal, save_path) -> None:
    import matplotlib
    matplotlib.use("Agg")
    ...
    ax.set_title("Square-lattice phononic crystal")
