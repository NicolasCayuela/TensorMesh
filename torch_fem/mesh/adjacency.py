...
def dense_connect(x:torch.Tensor)->torch.Tensor:
    ...
    parallel_dense = torch.vmap(dense) # [n_batch, n]->[n_batch, n^2, 2]
...
def node_adjacency(elements:torch.Tensor | Iterator[torch.Tensor],
    ...
    """
    dense_connect = lambda y: torch.vmap(lambda x:torch.stack(torch.meshgrid(x,x),-1))(y).reshape(-1, 2).T
...
def element_adjacency(elements:Dict[str,torch.Tensor])->SparseMatrix:
        ...
        for element_type, element_index in elements.items():
            ...
            if element.is_mix_facet:
                ...
                if Triangle in facet_type2facet:
                    facet_type2facet[Triangle] = torch.cat([facet_type2facet[Triangle], global_tri_facet], 0)
                    facet_type2element_ids[Triangle] = torch.cat([facet_type2element_ids[Triangle], _element_ids], 0)
                else:
                    facet_type2facet[Triangle] = global_tri_facet
                    facet_type2element_ids[Triangle] = _element_ids
                if Quadrilateral in facet_type2facet:
                    facet_type2facet[Quadrilateral] = torch.cat([facet_type2facet[Quadrilateral], global_quad_facet], 0)
                    facet_type2element_ids[Quadrilateral] = torch.cat([facet_type2element_ids[Quadrilateral], _element_ids], 0)
                else:
                    facet_type2facet[Quadrilateral] = global_quad_facet
                    facet_type2element_ids[Quadrilateral] = _element_ids
